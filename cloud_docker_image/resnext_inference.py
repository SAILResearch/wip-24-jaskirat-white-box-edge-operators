...
def resnext_int8_sptq_split_first_half_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_sptq_split_part1.onnx', providers=providers)
    result = session.run([session.get_outputs()[0].name], {
        session.get_inputs()[0].name: input_data})
    return result
...
def resnext_int8_sptq_split_second_half_single_inference(img):
    ...
    # get the name of the first input of the model
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_sptq_split_part2.onnx', providers=providers)
    result = session.run([session.get_outputs()[0].name], {
        session.get_inputs()[0].name: img[0]})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_int8_qat_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_qat.onnx', providers=providers)
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_pruned_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_pruned.onnx', providers=providers)
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_distilled_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext50_distilled.onnx', providers=providers)
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_distilled_int8_sptq_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext50_distilled_int8_sptq.onnx', providers=providers)
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result