...
def resnet_int8_sptq_split_first_half_single_inference(img):
    ...
    input_data = resnet_model.preprocess(image)
    session = ort.InferenceSession(
        'resnet_models/wide-resnet101_int8_sptq_split_part1.onnx', providers=['CPUExecutionProvider'])
    result = session.run([session.get_outputs()[0].name], {
        session.get_inputs()[0].name: input_data})
    return result
...
def resnet_int8_sptq_split_second_half_single_inference(img):
    ...
    # get the name of the first input of the model
    session = ort.InferenceSession(
        'resnet_models/wide-resnet101_int8_sptq_split_part2.onnx', providers=['CPUExecutionProvider'])
    result = session.run([session.get_outputs()[0].name], {
        session.get_inputs()[0].name: img[0]})
    result = resnet_model.postprocess(result)
    return result
...
def resnet_pruned_single_inference(img):
    ...
    input_data = resnet_model.preprocess(image)
    session = ort.InferenceSession(
        'resnet_models/wide-resnet101_pruned.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnet_model.postprocess(result)
    return result
...
def resnet_int8_qat_single_inference(img):
    ...
    input_data = resnet_model.preprocess(image)
    session = ort.InferenceSession(
        'resnet_models/wide-resnet101_int8_qat.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnet_model.postprocess(result)
    return result
...
def resnet_distilled_single_inference(img):
    ...
    input_data = resnet_model.preprocess(image)
    session = ort.InferenceSession(
        'resnet_models/wide-resnet50_distilled.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnet_model.postprocess(result)
    return result
...
def resnet_distilled_int8_sptq_single_inference(img):
    ...
    input_data = resnet_model.preprocess(image)
    session = ort.InferenceSession(
        'resnet_models/wide-resnet50_distilled_int8_sptq.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnet_model.postprocess(result)
    return result
